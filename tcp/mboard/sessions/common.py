...
def tcp_receive(sock,buffer_size=TCP_RECEIVE_BUFFER_SIZE):
    ...
    '''
    #This will not be a message in our case, but a file and we send the size with the request so no need to check the lengths. Read until EOF.
    #Reading from binary file always with a given amount of bytes. f = open('/tmp/test.txt', 'rb') -> f.read(5) -> 'Hello'
